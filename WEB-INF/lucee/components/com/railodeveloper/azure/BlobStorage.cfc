component {
	...
	public Void function writeFile(required String filepath, String filename, String mimeType)
	{
		...
		if (isNull(arguments.mimeType) or arguments.mimeType eq '') {
			arguments.mimeType = getMimeType(arguments.filepath);
		}
	...
	*/
	...
	public String function getMimetype(required string filepath) {
		/* Option 1: check the above list for the file extension */
		local.ext = listLast(filepath, '.');
		if (local.ext neq '' and structKeyExists(variables.ext2Mimetype, local.ext))
		{
